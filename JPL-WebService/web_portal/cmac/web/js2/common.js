...
// change_datan_
function change_datan(numTB, des_str, lonDf, latDf) {
  var nVar=Number( document.getElementById("nVar").value );
  //alert(nVar);

  var str1 = "";

  if (des_str == undefined) des_str = "Source Variable";
  if (lonDf == undefined) lonDf = [];
  if (latDf == undefined) latDf = [];

  for (var i=0; i<nVar; i++) {
    str1 = data_block_str(String(i+2), numTB,  des_str + String(i+1), "", 500, lonDf, latDf);
    //alert(str1);
